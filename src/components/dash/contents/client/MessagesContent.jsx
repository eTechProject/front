...
export default function MessagesContent() {
    ...
    /**
     * Gère la responsivité automatique
     */
    useEffect(() => {
        const handleResize = () => {
    ...
    return (
        <div className="absolute inset-0 flex bg-[#f7f7f8] rounded-xl border border-gray-100 overflow-hidden">
            ...
            <div className={`
                ...
                lg:translate-x-0 
                ${selectedUser && window.innerWidth < 1024 ? 'fixed' : 'relative'} 
                lg:relative
                    ...
                    {isLoading ? (
                        ...
                        filteredUsers.map((userData) => (
                            <div
                                ...
                                className={`cursor-pointer transition-colors ${selectedUser?.user?.id === userData.user.id ? 'bg-gray-100' : 'hover:bg-gray-50'}`}
                                onClick={() => handleSelectUser(userData)}
                            >
                                <div className="flex items-center gap-3 p-4">
                                    <div className="relative flex-shrink-0">
                                        <div
                                            className={`h-9 w-9 rounded-full flex items-center justify-center font-semibold text-base shadow-sm ${selectedUser?.user?.id === userData.user.id ? 'bg-gray-900 text-white' : 'bg-gray-200 text-gray-600'}`}>
                                            {userData.user.username.charAt(0).toUpperCase()}
                                        </div>
                                        <span
                                            className={`absolute -bottom-0.5 -right-0.5 h-2.5 w-2.5 border-2 border-white rounded-full ${userData.user.isOnline ? 'bg-green-400' : 'bg-gray-300'}`}></span>
                                    </div>
            ...
            <div className={`
                ...
                ${showSidebar && selectedUser ? 'hidden lg:flex' : 'flex'}
            `}>
                {!selectedUser ? (
                    ...
                    <>
                        <div className="border-b border-gray-100 px-4 lg:px-6 py-3 lg:py-4 flex items-center gap-3 bg-white">
                            ...
                            <div className="min-w-0 flex-1">
                                <div className="text-sm lg:text-base font-semibold text-gray-900 truncate">{selectedUser.user.username}</div>
