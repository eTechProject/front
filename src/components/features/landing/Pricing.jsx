...
import PricingSkeleton from "@/components/features/landing/skeleton/PricingSkeleton.jsx";

const Pricing = () => {
    const { isAuthenticated, setIntendedRedirect } = useAuth();
    const navigate = useNavigate();
    const [isLoading, setIsLoading] = useState(true);

    const { packs, fetchPacks } = usePack();
    useEffect(() => {
    ...
    const handleButtonClick = (packId) => {
        const intendedUrl = `/payment?plan=${encodeURIComponent(packId)}`;

        if (isAuthenticated) {
            navigate(intendedUrl);
        } else {
            setIntendedRedirect(intendedUrl);
            navigate("/auth");
        }
    };
    ...
    const sortedPacks = [...packs].sort((a, b) => a.nbAgents - b.nbAgents);
    const [starterPack, professionalPack] = sortedPacks;
        ...
        <section id="pricing" className="py-20 bg-zinc-900 px-4 sm:px-6 lg:px-8 relative overflow-hidden">
            ...
            <div className="max-w-7xl mx-auto relative z-10">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-5xl mx-auto">
                    {/* Starter Pack (toujours celui avec le moins d'agents) */}
                    {starterPack && (
                    ...
                    `,
                            ...
                            <div className="text-4xl font-bold text-white mb-2 relative z-10">
                                {starterPack.prix}€<span className="text-lg text-zinc-400">/mois</span>
                            </div>
                            <ul className="space-y-3 mb-8 relative z-10">
                                <li className="flex items-center text-zinc-300">
                                    <span className="text-[#FF8C00] mr-2">✓</span> Jusqu'à {starterPack.nbAgents} agents
                                </li>
                                {starterPack.description?.split(", ").map((item, i) => (
                                    <li key={i} className="flex items-center text-zinc-300">
                                        <span className="text-[#FF8C00] mr-2">✓</span> {item}
                                    </li>
                                ))}
                            </ul>
                            <button
                                onClick={() => handleButtonClick(starterPack.id)}
                                className="w-full bg-white text-zinc-900 py-3 rounded-full font-bold hover:bg-zinc-200 transition duration-300 relative z-10"
                            >
                                Choisir
                            </button>
                        </div>
                    ...
                    `,
                            ...
                            <div className="text-4xl font-bold text-white mb-2 relative z-10">
                                {professionalPack.prix}€<span className="text-lg text-zinc-400">/mois</span>
                            </div>
                            <ul className="space-y-3 mb-8 relative z-10">
                                <li className="flex items-center text-zinc-300">
                                    <span className="text-[#FF8C00] mr-2">✓</span> Jusqu'à {professionalPack.nbAgents} agents
                                </li>
                                {professionalPack.description?.split(", ").map((item, i) => (
                                    <li key={i} className="flex items-center text-zinc-300">
                                        <span className="text-[#FF8C00] mr-2">✓</span> {item}
                                    </li>
                                ))}
                            ...
                            <button
                                onClick={() => handleButtonClick(professionalPack.id)}
                                className="w-full bg-[#FF8C00] text-white py-3 rounded-full font-bold hover:bg-[#E67E00] transition duration-300 relative z-10"
                            >
                                Choisir
                            </button>
                        </div>
