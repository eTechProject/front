...
export default function SidebarAgent({user, logout}) {
    ...
    const Tooltip = ({children, text, className = ""}) => (
        <div className={`relative z-[999] group ${className}`}>
            {children}
            <div
                className="absolute left-full top-1/2 transform -translate-y-1/2 ml-3 pointer-events-none z-50 group-hover:opacity-100 opacity-0 transition-all duration-200 ease-out">
                <div
    ...
    return (
        <div className="flex h-screen bg-gray-50">
            {/* Sidebar */}
            <div
                className="w-20 bg-white  shadow-sm border-r border-gray-100 flex flex-col items-center py-6 relative z-30">
                ...
                `}
                            >
                                <div
                                    className="w-10 h-10 bg-gradient-to-br from-orange-400 to-orange-500 rounded-lg flex items-center justify-center text-white font-semibold text-sm shadow-inner">
